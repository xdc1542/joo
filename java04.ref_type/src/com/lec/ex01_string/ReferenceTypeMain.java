...
/*
	1. 데이타타입분류
	   ...
	   2) 참조타입(reference type)
	      ...
	      객체가 저장되어 있는 메모리의 주소를 참조하는 타입으로 Array(배열), Enum(열거),
	   ...
	   3) 기본 vs 참조
	      ...
	      Java는 문자리터럴이 동일하다면 String리터럴을 공유하도록 설계되어 있다. 일반적으로 변수에
	      문자를 저장할 경우에는 문자열리터럴을 사용하지만 객체 생성연산자인 new를 사용해서 직접
	      String객체를 생성할 수 있다.
	...
	2. 메모리사용영역
	   ...
	   2) Heap(힙)영역
	      ...
	      없는 쓰레기 객체가 되기 때문에 이런 객체를 쓰레기(garbage)로 취급되로 JVM의 쓰레기수집기
...
public class ReferenceTypeMain {
	...
	public static void main(String[] args) {
		...
		apt32 = null; // apt32변수는 더 이상 참조하는 객체(삼성레미안32평)가 없기 때문에 그 객체는 쓰레기가 된다.
		...
		System.out.println(person1 == person2);  // true or false? 리터럴 값을 비교 -> true
		System.out.println(person1 == person3);  // true or false? 값과 참조하는 메모리주소를 비교 -> false
		System.out.println(person1 == person4);  // true or false? 값과 참조하는 메모리주소를 비교 -> false
		System.out.println(person3 == person4);  // true or false? 서로 다른 객체의 참조주소를 비교 -> false
		System.out.println();
