...
/*
	...
	1. 필드(Field, 속성, 클래스변수)
	   ...
	   변수는 생성자와 메서드내에서만 사용되고 생성자와 생성자와 메서드의 실행이 종료가 되면 자동으로
	   소멸이 되지만 필드는 생성자와 메서드 전체에서 사용(전역변수개념)되고 객체가 소멸되지 않는 한
	...
	3. 생성자(Constructor, 객체초기화)
	   ...
	   초기화를 담당한다. 즉, 필드를 초기화하거나 메서드를 호출해서 객체를 생성할 수 있도록 한다.
	   
	   1) 기본생성자(default Constructor)
	   
	      모든 클래스는 생성자가 반드시 존재해야 하며 한 개이상의 생성자를 가질 수 있다. 단, 복수의 
	      생성자를 정의할 경우에는 매개변수의 갯수, 데이터타입 및 순서가 달라야 한다. 즉, 동잏한 형태의
	      시그니쳐를 갖는(즉, 중복되는) 생성자는 정의할 수 없다.
	      
	      기본 생성자는 별도로 정의하지 않아도 자바컴파일러(javac.exe)가 컴파일시에 자동으로 추가한다.
	      하지만, 개발자가 기본생성자이외의 생성자를 정의할 경우에는 자동으로 추가되지 않는다. 따라서
	      기본생성자이외의 다를 생성자를 정의하고 기본생성자가 필요할 경우에는 반드시 명시적으로 기본
	      생성자를 정의해야 한다.
...
public class Car {
	...
	private Car(String company, String model, String color, int maxSpeed, int speed) {
		super();
