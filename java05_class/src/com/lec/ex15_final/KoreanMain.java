...
/*
	...
	2. static final 필드
	   ...
	   final필드는 객체마다 저장되고 생성자의 매개값을 통해서 여러가지 값으을 객체별고 독립적
	   으로 가질 수 있기 때문에 상수가 될 수 없다.
	   
	   그래서 Java에서는 상수는 final이면서 static으로 선언된다. static은 정적멤버이기 때문에
	   객체마다 저장되지 않고 클래스에 포함된다. 그리고 한번, 초기화가 되면 그 이후에는 더이상
	   수정할 수 없다.
*/
...
class Korean {
	...
	static final String NATION = "대한민국"; // 1. 1st 초기화
	String name;
	final String ssn;
	
	// public Korean() {} // final필드가 있는 경우 그 필드가 초기화되지 않았을 경우 기본생성자는 정의불가  
	public Korean(String name, String ssn) { // 2. 2nd 초기화 방법
		this.name = name;
		this.ssn = ssn;
	}
}
