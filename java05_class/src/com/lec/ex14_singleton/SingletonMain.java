...
/*
	...
	1. 외부에서 new객체생성연산로 생성자를 호출할 수 없도록 막아야한다.
	   그 이유는 생성자를 호출한 만클 무제한으로 객체가 생성되기 때문이다.
	   생성자를 외부에서 호출할 수 없도록 하려면 생성자에 접근을 하지 못
	   하도록 private접근제한자로 선언하는 것이다.
	
	2. 자신의 타입인 정적필드를 선언하고 자신의 객체를 생성해서 초기화 한 후
	   내부에서 new연산자로 객체를 생성하고 이 객체를 공통으로 사용하기 위해서
	   정적(static)필드로 선안하고 이 객체를 외부에서 접근하지 못하도록 
	   private접근제한자로 선언한다.
	  
	3. 새롭게 생성된 singleton객체를 외부에서 접근하는 메서드를 작성하고 그 
