...
/*
	...
	1. 추상클래스의 개념
	   ...
	   수가 있다. 추상클래스와 실체클래스는 상속관계를 가지고 있다. 추상클래스가 부모클래
	   스이고 실체 클래스가 자식클래스로 구현하여 실체 클래스는 부모인 추상클래스에 정의
	   된 모든 특성들을 상속할 뿐만 아니라 자식클래스에 새로운 특성들을 추가할 수가 있다.
	...
	2. 추상클래스의 용도
	   ...
	   2) 실체클래스들을 작성할 때 개발시간과 비용을 절약할 수가 있다.	
	...
	4. 추상메서드와 오버라이드
	   ...
	   하는 것이 좋을 것이지만 메서드의 선언만하고 실체 실행될 내용들은 실체클래스마다 달라
	   야 하는 경우가 많다.
	   
	   예를 들어 Animal이라는 추래서에서 sound()라는 메서드를 정의한 후에 Cat, Dog, Bird 즉,
	   Animal 추상클래스를 상속받은 실체클래스 Dog, Cat...에서 각각의 실체의 특성에 맞게 메
	   서드를 작성하고 sound()라는 메서드는 실체 클래스에서 반드시 구현되도록 강제화하기 위
	   해 추상메서드를 선언하고 그 추상메서드를 상속받는 실체 클래스에서 실제로 실행할 메서
	   드로 구현(override)하도록 한다.
*/
