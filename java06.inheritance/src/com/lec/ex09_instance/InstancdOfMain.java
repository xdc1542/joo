...
/*
	...
	instanceof연산자는 좌측항은 객체가 오고 우측한은 참조타입이 오는데 좌측항의 객체가
	우측항의 인스턴스(객체)가 아니면 false, 우측항의 타입으로 생성된 인스턴스(객체)라면
	true를 리턴한다.
	
	instanceOf연산자는 매개값의 타입을 조사할 때 주로 사요한다. 메서드내에서 강제타입변환이
	필요할 경우 반드시 매개값이 어떤 객체타입인지 instanceof연산자로 확인하고 안전하게 강제
	타입변환을 해야 한다.
	
	만약, 타입을 확인하지 않고 강제 타입변환을 시도할 경우, 강제타입변환이 되지 않을 경우에는
	ClassCastException예외(실행예외, Runtime Exception)가 발생한다.
*/
public class InstancdOfMain {
	...
	static void method2(Parent parent) {
		...
		if(parent instanceof Child) {
			Child child = (Child) parent;
			System.out.println("강제형변환성공!! - 중요한 로직이 성공적으로 실행되었습니다!");
		} else {
