...
%>
<!-- 
    톰캣에서 get/post 방식으로 전달되는 파라미터의 값을 읽어 올 때 사용하는
    문자셋은 기본적으로 utr-8로 전달된다. 그렇기 때문에 tomcat에서는 별도로
...
<body>
	...
	<%
	   ...
	   String[]values =map.get("name"); // name은 값이 아닌 속성으로 불러와서 값이 도출되는 것.
	   if(values != null){
	...
	<hr />
	<%
	   map = request.getParameterMap(); //중복선언
	   values =map.get("hobby"); // name은 값이 아닌 속성으로 불러와서 값이 도출되는 것.
	   if(values != null){
		   for(int i=0; i<values.length; i++){
	%>
