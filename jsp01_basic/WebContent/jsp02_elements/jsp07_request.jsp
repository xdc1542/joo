...
<!-- 
     ...
     request 기본객체는 jsp페이지에서 가장 많이 사용되는 객체로서 웹브라우저의 요청과 관련있는 객체이다. -> 웹브라우저에 있는 요청들은 모드 request에 들어온다.
     웹브라우저에서 웹사이트주소를 입력하면 웹브라우저는 해당주소의 웹서버에 연결한 후 요청정보를 전송
     하는 데 이 요청 정보를 웹서버에 전달하는 객체가 request객체이다.
     
     A. request객체가 제공하는 기능
        
        a. 클라이언트(웹브라우저(크롬 엣지 ...))와 관련된 정보
        b. 서버와 관련된 정보
     ...
     B. request객체의 메서드
        ...
        a. getRemoteAddr()         : 웹서버에 연결한 클라이언트 IP주소
        b. getContentLength()      : 클라이언트가 요청한 정보의 총 길이
        c. getCharacterEncoding()  : 클라이언트가 요청정보를 전송할 때 사용한 문자셋 정보
        ...
        g. getRequestURI()         : 웹브라우저가 요청한 URL정보
           // 쉽게 말해서 URL정보
     ...
     C. request객체의 요청파라미터 관련 메서드
        ...
        a. getParameter(String name)       : (name = name)name인 파라미터의 값을 리턴, 없을 경우 null
        b. getParameterValues(String name) : " name인 파라미터의 값을 배열로 리턴, 없을 경우 null
        c. getParameterNames()             : 웹브라우저가 전송한 파라미터의 이름목록을 리턴
