...
<!-- 
   ...
   1. 디렉티브(지시어, directive) - ex) @ : 지시어
      ...
      b. taglib  : jsp페이지에서 사용할 taglib를 제공
      c. include : jsp페이지에 다른 jsp페이지를 로딩
                   예를 들어서 header, footer처럼 고정적인 페이지를 공통으로 사용되는 페이지를 loading
                   
   2. 스크립트(script)
   
      JSP에서 jsp파일을 동적으로 생성하기 위해 사용하는 것으로 스크립트요소를 사용하면
      사용자가 jsp페이지에 입력한 정보를 DB에 저장, 검색, 출력 등을 할 수가 있다.
   ...
   3. 기본객체(내장객체, implict object)
      ...
      이 기본객체는 application, session, page, request, response등을 기본으로 제공되는 객체를 말한다.
      이러한 객체들을 각각의 객체마다 고유한 속성 또는 기능을 제공하는데 요청된 파라미터의 값을
      읽거나 응답결과를 설정, 세션정보의 처리등 웹애플리케이션에서 필요한 기능을 제공한다.
      
      a. application : 웹애플리케이션의 전반적인 정보를 저장
      b. session     : http세션 정보를 저장하는 객체
      c. page        : JSP페이지의 정보를 저장
      d. request     : 웹브라우저가 웹서버에 요청하는 정보를 전달하는 객체
      e. response    : 웹서버에 요청된 결과를 응답해주는 역할을 하는 객체
      f. out         : DOM를 출력하는 스티림객체
