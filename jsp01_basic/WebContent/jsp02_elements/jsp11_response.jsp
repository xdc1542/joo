...
<%-- JSP주석 --%>
<!--  
	response 기본객체(HttpServletReponse)
	
	response객체는 request객체와 반대기능을 가지고 있다. request객체가 웹브라우저로 전송한
	요청정보를 저장하고 있다면 response객체는 웹서버가 웹브라우저로 전송한 응답정보를 가지
	고 있다.
	...
	A. response객체의 메서드
	   ...
	   d. setHeader(String name, String vlaue)  : name이라는 헤더에 String 타입의 값을 저장(수정)
	   e. setIntHeader(String name, int value)  : name이라는 헤더에 int 타입의 값을 저장
	   f. setDateHeader(String name, long value): name이라는 헤더에 long 타입의 date값을 젖정
	
	B. 웹브라어저에 Cache제어 관련 응답헤더
	   ...
	   캐시(Cache)란? 웹브라우저가 WAS에 jsp실행을 요청하고 잠시 뒤에 동일한 JSP 실행을 요청하는
	   경우에 첫 번째요청과 두 번째 요청사이에 결과값이 차이가 없을 경우에 불필요한 응답결과를 
	   반복해서 요청하는 셈이 된다. 캐시는 이렇게 동일한 데이터를 중복해서 로딩하지 않도록 할 경우
	   에 사용한다. 웹브라우저는 첫 번째 요청시에 응답결과를 로컬PC의 임시저장소인 캐시에 저장한다.
	   
	   이후에 동일한 URL에 대한 요청이 있을 경우에 WAS에 접근하지 않고 로컬PC에 저장된 응답결과를
	   사용한다. 캐시에 보관된 데이터를 사용할 경우 서버에 접근하지 않기 때문에 휠씬 빠른 결과를
	   웹브라우저에 출력할 수 있게 된다.
	...
	D. 리다이렉트(redirect)
	   ...
	   response객체는 sendRedirect(String location)메서드릉 이용해서 웹브라우저가 다른 페이지로 
	   이동하도록 지시할 수 있다.
-->
<%
...
<body>
	<h1 class="primary">아이디가 null이거나 잘못된 아이디 입니다!!</h1>
</body>
</html>
<%		
	}
